...
// 第 2 天、第 3 天、第 4 天、第5 天、第 6 天、第 7 天和第 8 天。
//
...
class Solution {
    ...
    public int mincostTickets(int[] days, int[] costs) {
        this.days = days;
        this.costs = costs;
        value = new int[days.length];
        return totalCost(0);
    }

    public int totalCost(int i) {
        if (i >= days.length) {
            return 0;
        }
        if (value[i] != 0) {
        ...
        for (int j = 0; j < interval.length; j++) {
            int k = i+1;
            while (k < days.length  && days[k] < days[i] + interval[j]) {
                k++;
            }
            value[i] = Math.min(value[i],totalCost(k)+costs[j]);
        }
